...
for (i in 1:repeat_time_in){
  kernel_result <- data.frame(matrix(0, repeat_time_in, 3+length(band_coef)*2+4))
  colnames(kernel_result) <- c("pvalue_standard", 
                                "X_standard",
                                "pvalue_naive", 
  ...
  kernel_result[i,1] <- pvalue_standard
  kernel_result[i,2] <- X_standard
  ...
  kernel_result[i,3] <- pvalue_naive
  ...
  for(j in 1:length(band_coef)){
    ...
    kernel_result[i,3+j] <- pkde(X_standard_transform,lower.tail = F) 
    MY_KDE2 <- KDE(X_perm_each_sub_transform, bandwidth = band_coef[j]*bw.nrd0(X_perm_each_sub_transform), to.environment = T) 
    kernel_result[i,3+length(band_coef)+j] <- pkde(X_standard_transform,lower.tail = F)
  }
  
  kernel_result[i,3+length(band_coef)*2+1] <- skewness(X_perm_each_transform)
  kernel_result[i,3+length(band_coef)*2+2] <- kurtosis(X_perm_each_transform)
  kernel_result[i,3+length(band_coef)*2+3] <- skewness(X_perm_each_sub_transform)
  kernel_result[i,3+length(band_coef)*2+4] <- kurtosis(X_perm_each_sub_transform)
  
  write.csv(kernel_result, "SKAT_KDE_original.csv", row.names=FALSE)
  ...
  write.csv(kernel_result, "SKAT_KDE_square_root.csv", row.names=FALSE)
  ...
  write.csv(kernel_result, "SKAT_KDE_cubic_root.csv", row.names=FALSE)
  ...
  write.csv(kernel_result, "SKAT_KDE_log.csv", row.names=FALSE)
  ...
  if(lambda_X == 0){
    ...
    write.csv(kernel_result, "SKAT_KDE_Box.csv", row.names=FALSE)
  } else {
    lambda_X_optinal <- lambda_X
  ...
  kernel_result$lambda_optinal <- lambda_X_optinal
  write.csv(kernel_result, "SKAT_KDE_Box_Skew.csv", row.names=FALSE)
  ...
  kernel_result$lambda_optinal <- lambda_X_optinal
  write.csv(kernel_result, "SKAT_KDE_Box_Kurt.csv", row.names=FALSE)
